...
sink(file="macula_big.class_071424.txt")
...
bk.dat <- read.csv("bulk_macular_subset_counts.tsv", sep="\t", row.names=1)

## Single cell reference data ##
# Rows = bulk cell IDs; Columns = gene names or IDs
sc.dat <- read.csv("downsample5k_macula_071124.csv", row.names=1)

## Cell type labels ##
# Should be the same length as nrow(sc.dat)
meta = read.csv("meta_downsample5k_macula_071124.csv", row.names=1)
